...
```
* Mover los archivos a una ubicación en el PATH _/usr/local/bin/_ para hacerlos ejecutables:
```bash
...
```
***
### Apartir de acá se siguió el tutorial de la [página web de TTN](https://www.thethingsindustries.com/docs/enterprise/docker/configuration/), los cuales se resumen a continuación:
...
```
Ahora crear el archivo `cert.json`:
```bash
cat << EOF > cert.json
{
  "hosts": ["192.168.137.28"],
  "names": [
    {
      "C": "NL",
      "ST": "Noord-Holland",
...
EOF
```
**Nota**: *Recuerde reemplazar `192.168.137.28` con la dirección de su servidor!*
...
```
Al final, borrar los archivos necesarios, de modo que su directorio se vea así:
```
cert.pem
key.pem
ca.pem
...
**Nota**: *Verificar previamente si posee el comando `docker compose` o `docker-compose` (adecuar los siguientes comandos si tiene `docker-compose`), además revisar si está agregado el usuario al grupo de docker para poder ejecutar comandos sin la necesidad de anteponer sudo (se puede probar haciendo `docker ps`, si no necesita sudo está listo, de lo contrario debe anteponer sudo cada vez que haya un comando de docker)*  
...
```bash
docker compose run --rm stack is-db create-admin-user --id admin --email admin@192.168.137.28
```
**Nota**: *Cambiar la IP de ser necesario, además no toma todas las contraseñas, `Admin1` la toma bien. Usar alguna parecida o esa misma.*
***
...
```
***
- Crear un cliente OAuth para la consola (reemplace con su SERVER_ADDRESS y Consola CLIENT_SECRET):
```bash
SERVER_ADDRESS=https://192.168.137.28
ID=console
NAME=Console
...
Va a mostrar que la página no es segura, permitir en configuración avanzada. Una vez en la interfaz web de TTN, poner como usuario `admin` y la contraseña que se escribió, por ejemplo `Admin1` si usó esta.
